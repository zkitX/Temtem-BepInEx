...
namespace TemTemMod.Mods
{
    ...
    {
        private static LocalPlayerAvatar localplayeravatar;
        void Update()
        {
            var position = LocalPlayerAvatar.nkqrjhelndm.qqhqkomhdoq;
            if (Input.GetMouseButtonDown(1))
            {
                ...
                if (obj != null && obj.transform.GetChild(0).gameObject.activeInHierarchy)
                {
                    ...
                    NetworkLogic.nkqrjhelndm.elennjqknrp(NetworkLogic.nkqrjhelndm.npqcecmqpio, navHit.position);
                }
                else
                {
            ...
            if (Input.GetKeyDown(KeyCode.Keypad4))
                NetworkLogic.nkqrjhelndm.elennjqknrp(NetworkLogic.nkqrjhelndm.npqcecmqpio, new Vector3(position.x - teleportDistance, position.y, position.z));
            if (Input.GetKeyDown(KeyCode.Keypad6))
                NetworkLogic.nkqrjhelndm.elennjqknrp(NetworkLogic.nkqrjhelndm.npqcecmqpio, new Vector3(position.x + teleportDistance, position.y, position.z));
            if (Input.GetKeyDown(KeyCode.Keypad8))
                NetworkLogic.nkqrjhelndm.elennjqknrp(NetworkLogic.nkqrjhelndm.npqcecmqpio, new Vector3(position.x, position.y, position.z + teleportDistance));
            if (Input.GetKeyDown(KeyCode.Keypad2))
                NetworkLogic.nkqrjhelndm.elennjqknrp(NetworkLogic.nkqrjhelndm.npqcecmqpio, new Vector3(position.x, position.y, position.z - teleportDistance));
            if (Input.GetKeyDown(KeyCode.Keypad7))
                NetworkLogic.nkqrjhelndm.elennjqknrp(NetworkLogic.nkqrjhelndm.npqcecmqpio, new Vector3(position.x, position.y + teleportDistance, position.z));
